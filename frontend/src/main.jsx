...
function RiskChart({ history=[], events=[], entrySpot=null }) {
  ...
  return <div className="chart-wrap">
    <div className="chart-heading"><div><strong>Risk timeline</strong><span>Risk starts at 0 at entry. Spot change is measured from the captured entry spot.</span></div><div className="chart-entry">Entry spot <b>{baseSpot ? '₹'+num(baseSpot) : '—'}</b></div></div>
...
function RiskPanel({ strategy }) {
  ...
  return <div className="risk-panel">
    ...
    <div className="risk-section"><div className="risk-section-title"><strong>Market position</strong><span>Current values</span></div><div className="risk-grid">
      <div className="risk-card primary-metric"><span>Underlying spot</span><strong>{risk.spot==null?'Waiting…':'₹'+num(risk.spot)}</strong><small>Entry: {risk.entry_spot==null?'Waiting…':'₹'+num(risk.entry_spot)} · Change: {pct(risk.spot_change_pct)}</small></div>
    ...
    <div className="risk-section"><div className="risk-section-title"><strong>Technical context</strong><span>Underlying 1-minute data</span></div><div className="tech-row">{Object.entries(risk.technical||{}).map(([k,v])=><span key={k}>{k.replaceAll('_',' ')} <b>{num(v,2)}</b></span>)}</div></div>
